...
function createComputation0(i) {
    S.run(function () { return i; });
}

function createComputation1(s1) {
    S.run(function () { return s1.get(); });
}

function createComputation2(s1, s2) {
    S.run(function () { return s1.get() + s2.get(); });
}

function createComputation4(s1, s2, s3, s4) {
    S.run(function () { return s1.get() + s2.get() + s3.get() + s4.get(); });
}

function createComputation8(s1, s2, s3, s4, s5, s6, s7, s8) {
    S.run(function () { return s1.get() + s2.get() + s3.get() + s4.get() + s5.get() + s6.get() + s7.get() + s8.get(); });
}
...
function updateComputations1to1(n, sources) {
    var s1 = sources[0],
        c = S.run(function () { return s1.get(); });
    for (var i = 0; i < n; i++) {
        s1.set(i);
    }
}
...
function updateComputations2to1(n, sources) {
    var s1 = sources[0],
        s2 = sources[1],
        c = S.run(function () { return s1.get() + s2.get(); });
    for (var i = 0; i < n; i++) {
        s1.set(i);
    }
}
...
function updateComputations4to1(n, sources) {
    var s1 = sources[0],
        ...
        s3 = sources[2],
        s4 = sources[3],
        c = S.run(function () { return s1.get() + s2.get() + s3.get() + s4.get(); });
    for (var i = 0; i < n; i++) {
        s1.set(i);
    }
}
...
function updateComputations1to2(n, sources) {
    var s1 = sources[0],
        c1 = S.run(function () { return s1.get(); }),
        c2 = S.run(function () { return s1.get(); });
    for (var i = 0; i < n / 2; i++) {
        s1.set(i);
    }
}
...
function updateComputations1to4(n, sources) {
    var s1 = sources[0],
        c1 = S.run(function () { return s1.get(); }),
        c2 = S.run(function () { return s1.get(); }),
        c3 = S.run(function () { return s1.get(); }),
        c4 = S.run(function () { return s1.get(); });
    for (var i = 0; i < n / 4; i++) {
        s1.set(i);
    }
}
